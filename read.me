Окей! Ниже — рабочий автокликер на Python с выбором области, количеством кликов и «амплитудой» (джиттер по пикселям). Есть два режима:

* **Область (`--area` / `--pick-area`)** — каждый клик в случайной точке внутри прямоугольника.
* **Точка + амплитуда (`--point` / `--pick-point`)** — целимся в точку, добавляем случайное смещение в пределах амплитуды (px).

Есть задержка перед стартом, рандомизация интервала между кликами, выбор кнопки мыши. Экстренный стоп: быстро уведите курсор в **левый верхний угол** — это failsafe PyAutoGUI.
...
1. **Кликать внутри выбранной области** (интерактивный выбор углов):

```bash
python clicker.py --pick-area --count 200 --cps 12 --interval-jitter 0.02 --start-delay 5
```

2. **Кликать вокруг точки с амплитудой 25 px** (интерактивно выбрать точку):

```bash
python clicker.py --pick-point --amplitude 25 --count 150 --interval 0.08 --interval-jitter 0.01 --button left
```

3. **Задать область числами**:

```bash
python clicker.py --area 100 200 500 600 --count 300 --cps 10
```
...
* На **macOS** при первом запуске может потребоваться дать приложению (терминалу/IDE) доступ в *System Settings → Privacy & Security → Accessibility*.
* Экстренный стоп всегда работает: **двиньте мышь в левый верхний угол**.
* Если нужно — добавлю горячие клавиши «Старт/Стоп» через `pynput`.
